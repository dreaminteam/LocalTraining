...
CREATE TABLE IF NOT EXISTS `races`.`employee` (
  ...
  INDEX `fk_employee_user1_idx` (`login` ASC)  COMMENT '',
  CONSTRAINT `fk_employee_user1`
    FOREIGN KEY (`login`)
    REFERENCES `races`.`user` (`login`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `races`.`player` (
  ...
  PRIMARY KEY (`id`)  COMMENT '',
  UNIQUE INDEX `id_UNIQUE` (`id` ASC)  COMMENT '',
  INDEX `fk_player_user1_idx` (`login` ASC)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `races`.`access_level` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT '',
  `dexcription` TEXT(1000) NOT NULL COMMENT '',
  PRIMARY KEY (`id`)  COMMENT '',
  UNIQUE INDEX `id_UNIQUE` (`id` ASC)  COMMENT '')
ENGINE = InnoDB;


CREATE TABLE IF NOT EXISTS `races`.`permission` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT '',
  `access_level_id` INT NOT NULL COMMENT '',
  `employee_id` INT NOT NULL COMMENT '',
  INDEX `fk_permission_access_level1_idx` (`access_level_id` ASC)  COMMENT '',
  PRIMARY KEY (`id`)  COMMENT '',
  UNIQUE INDEX `id_UNIQUE` (`id` ASC)  COMMENT '',
  INDEX `fk_permission_employee1_idx` (`employee_id` ASC)  COMMENT '',
  CONSTRAINT `fk_permission_access_level1`
    FOREIGN KEY (`access_level_id`)
    REFERENCES `races`.`access_level` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_permission_employee1`
    FOREIGN KEY (`employee_id`)
    REFERENCES `races`.`employee` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
